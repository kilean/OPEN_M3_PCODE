    ...
    public partial class Fo_Main : Form
    {
        ...
        */
        ...
        private void pic_MacroLimit_Click(object sender, EventArgs e)
        {
            ...
            cb_SelectGwMarco.Items.Clear();
            string[] gwName = new string[4] { "砂輪1", "砂輪2", "砂輪3", "砂輪4"};
            for(int i = 0; i < GwCount; i ++)
            {
                if(i < gwName.Length)
                {
                    cb_SelectGwMarco.Items.Add(gwName[i]);
                }
            }
            cb_SelectGwMarco.SelectedIndex = 0;
        ...
        private void ShowMacriLimit(XmlElement root, int gwNo)
        {
            int shift = gwNo * 200;
            foreach (XmlElement x in root.ChildNodes)
            {
                ...
                double.TryParse(x.GetAttribute("Min"), out double min);
                string Unit = x.GetAttribute("Unit");
                dgv_MacroLimit.Rows.Add(no + shift, name, min, max, Unit, x);
            }
        }
        private void cb_SelectGwMarco_SelectedIndexChanged(object sender, EventArgs e)
        {
            ComboBox cb = (ComboBox)sender;
            if (cb.SelectedIndex < 0) return;
            dgv_MacroLimit.Rows.Clear();
            ShowMacriLimit(xmlMacroDoc.DocumentElement, cb.SelectedIndex);
        }
